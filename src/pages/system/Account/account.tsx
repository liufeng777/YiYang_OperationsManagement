...
export default function AccountList() {
  ...
  const saveAccount = (account: AccountItem) => {
    setData((prev) =>
  ...
  /** 停用/启用二次确认 */
  const confirmToggle = (record: AccountItem) => {
    const toDisable = record.status === 1
    modal.confirm({
      ...
      onOk: () => {
        ...
        message.success(`${record.username} 已${toDisable ? '停用' : '启用'}（mock）`)
      },
    })
  }
  ...
  const confirmDelete = (record: AccountItem) => {
    modal.confirm({
      ...
      onOk: () => {
        ...
        message.success(`用户 ${record.username} 已删除（mock）`)
      },
    })
  }

  /** 重置密码二次确认 */
  const confirmResetPassword = (record: AccountItem) => {
    modal.confirm({
      title: `确认重置用户「${record.nickname}」的密码？`,
      content: '重置后密码将恢复为初始密码。',
      okText: '重置',
      cancelText: '取消',
      onOk: () => {
        message.success(`用户 ${record.username} 的密码已重置为初始密码（mock）`)
      },
    })
  }
      ...
      { title: '最近登录', dataIndex: 'last_login_at', key: 'last_login_at', width: 160, render: (v: number) => formatDateTime(v * 1000)},
      {
        ...
        render: (_, record) => (
          <div className="account-actions">
            ...
            <Button type="link" size="small" onClick={() => confirmResetPassword(record)}>
              重置密码
            </Button>
